...
"""
# Most of this part is about old method
...
class DynamicsModel:
    ...
    def get_dynamics(self):
        ...
        if self.env.dynamics_mode == 'Unicycle':
            ...
            def get_g(state_batch, t_batch=None):
                g_x = np.zeros((state_batch.shape[0], 6, 2))
                theta = float(state_batch[:, 2])
                g_x = np.array([[[0, 0], [0, 0], [0, 0],
                                [-np.sin(theta)/self.env.mass, -np.sin(theta)/self.env.mass],
                                [np.cos(theta)/self.env.mass, np.cos(theta)/self.env.mass],
                                [       -self.env.d/self.env.Iyy,         self.env.d/self.env.Iyy]]])
                return g_x
        else:
    ...
    def append_transition(self, state_batch, u_batch, next_state_batch, t_batch=None):
        ...
        # print('disturbance_batch = ', disturbance_batch)
