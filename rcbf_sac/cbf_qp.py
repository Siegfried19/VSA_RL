...
class CascadeCBFLayer:
    ...
    def get_u_safe(self, u_nom, s, mean_pred, sigma, use_L1=False, disturb=None):
        ...
        P, q, G, h = self.get_cbf_qp_constraints(u_nom, s, mean_pred, sigma, use_L1, disturb)
        u_safe = self.solve_qp(P, q, G, h)

        return u_safe

    def get_cbf_qp_constraints(self, u_nom, state, mean_pred, sigma_pred, use_L1=False, disturb=None):
        ...
        if self.env.dynamics_mode == 'Unicycle':
            ...
            for c in range(hs.shape[0]):
                ...
                h[ineq_constraint_counter] = (self.gamma_b * (h_ ** 3) + np.dot(dhdx_, (f_p+mean_p)) + np.dot(dhdx_ @ g_p,u_nom) \
                                             - self.k_d * np.dot(np.abs(dhdx_), sigma_p))[0]*use_L1
            ...
            f_x[2] = state[5]
            f_x[4] = -self.env.g
            fD_x = np.zeros(state.shape[0])
            fD_x[3:5] = sigma_pred[3:5, 0]

            # g(x)
            g_x = np.zeros((state.shape[0], n_u))
            g_x[3, 0] = -np.sin(theta)/self.env.mass
            g_x[3, 1] = -np.sin(theta)/self.env.mass
            g_x[4, 0] = np.cos(theta)/self.env.mass
            g_x[4, 1] = np.cos(theta)/self.env.mass
            ...
            dLfhdx[3] = -state[0]
            dLfhdx[4] = -state[1]
            Lffh = np.dot(dLfhdx, f_x)
            LDffh = np.dot(dLfhdx, fD_x)

            # Lfgh
            Lgfh = np.dot(dLfhdx, g_x)
            if use_L1:
                L1_penalty = self.env.L1_gamma #self.env.L1_theta - 2*self.env.L1_gamma 
                L1_penalty = np.linalg.norm(dLfhdx) * L1_penalty
                # print(L1_penalty)
                LDffh = np.dot(dLfhdx, mean_pred)
                h[0] = Lffh + (self.gamma_b + self.gamma_b) * dh_cbf + self.gamma_b * self.gamma_b * h_cbf + np.dot(Lgfh, u_nom).squeeze() + LDffh - L1_penalty
            else:
                h[0] = Lffh + (self.gamma_b + self.gamma_b) * dh_cbf + self.gamma_b * self.gamma_b * h_cbf + np.dot(Lgfh, u_nom).squeeze() - LDffh
            
            G[0, 0] = -Lgfh[0]
            G[0, 1] = -Lgfh[1]
            G[:1, n_u] = -2e2  # for slack
            ineq_constraint_counter += 1
            P = np.diag([0.01, 0.01, 1.5e2])
            q = np.zeros(n_u + 1)
