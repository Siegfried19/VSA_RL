...
class RCBF_SAC(object):
    ...
    def load_weights(self, output):
        ...
        # )
        self.policy.load_state_dict(
            torch.load('{}/actor.pkl'.format(output), map_location=torch.device('cuda:0'))
        )
        self.critic.load_state_dict(
    ...
    def get_safe_action(self, obs_batch, action_batch, dynamics_model, sigma_hat):
        ...
        if self.use_L1:
            ...
            if len(mean_pred_batch.shape) == 1:
                ...
                sigma_pred_batch = sigma_pred_batch.unsqueeze(0)
        else:
            mean_pred_batch, sigma_pred_batch = dynamics_model.predict_disturbance(state_batch)
        # For L1, mean_pred_batch is the disturbance itself, sigma_pred_batch is zeros
        safe_action_batch = self.cbf_layer.get_safe_action(state_batch, action_batch, mean_pred_batch, sigma_pred_batch, self.use_L1)
